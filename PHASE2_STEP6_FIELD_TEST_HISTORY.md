...
## 4.23 2026-05-29 Debug 실측 — VFS 핵심 시나리오 안정 (commit `7ecf01c`)

### A) 사용자·로그 판정

- **시나리오**: `test_01` / `test_02` — 열기 → 편집 → QSAVE → 닫기 → 재오픈(반복), EPDF → 저장 → 닫기 → 재오픈
- **PID 24368** (`vfs_console_24368.log`, 17:35~17:37): 프리징 0, `materialize 0 bytes` 0, Open 실패 0, `L1 canonical disk vaporized (CadOpen)` 8/8
- **판정**: Debug \| x64 — **I1·I2·I3·I5 기능 게이트 통과**(G4·L3 정식·L4 종료 기화는 미완)

## 4.24 2026-05-29 Release MSI 스모크 — 배포 경로 + `messages.xml`

### A) 사용자 판정

- **빌드**: `Release \| x64` Main → `eDIAN.Setup.msi` 설치 후 ZWCAD 실측
- **VFS**: Debug와 **동일** — Open / QSAVE / 닫기 / 재오픈 정상
- **UI(초기 NG)**: 팔레트 닫기 시 `confirm.save.document` 문구 미표시(파일명만 표시)
- **UI(수정 후 OK)**: MSI에 `Resources\global\messages.xml` 포함 재설치 → **「저장하시겠습니까?」** 정상

### B) 원인·수정

| 항목 | 내용 |
|------|------|
| 원인 | `eDIAN.Setup.vdproj`가 `ProjectOutput(Built)`만 포함 — `CopyToOutputDirectory` Content(`messages.xml`) **MSI 미포함** |
| 수정 | vdproj: `TARGETDIR\Resources\global\messages.xml` 명시 추가 |
| 진단 | `Global.loadMessage` — 파일 없을 때 `application.log` 경고 1줄 |

### C) 판정

- **Release MSI 스모크**: **성공** (배포·VFS·MIP 닫기 UI)
- **잔여**: G4 노출 최소화(L3 정식), L4 EPDF 종료 기화, L7 AutoCAD 회귀, 회귀 체크리스트 전항목(§`ROADMAP_PHASE2_VFS.md` §6)
